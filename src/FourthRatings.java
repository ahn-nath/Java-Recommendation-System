...
public class FourthRatings {
	...
	private int dotProduct(Rater me, Rater r) {
		...
		HashMap<String, Rating> myRatings = getMyRatings(me);
		HashMap<String, Rating> theirRatings = getMyRatings(r);
	...
	private double getAverageByIDTop(String movieId, int minimalRaters, ArrayList<Rating> listRaters,
			...
			if (listRaters.size() >= numSimilarRaters) {
			...
			for (Rating each : topRaters) {
				...
				HashMap<String, Rating> raterMovieRatings = getMyRatings(rater);
	...
	private double getAverageByID(String id, int minimalRaters) {
		...
		List<HashMap<String, Rating>> allRatings = myRaters.stream().map(Rater::getMyRatings)
		...
		if (matches.size() >= minimalRaters) {
			...
			List<Double> allScores = matches.stream().map(Rating::getValue).collect(Collectors.toList());
	...
	// return ratings for Rater object
	public HashMap<String, Rating> getMyRatings(Rater rater) {

		// getRatings method
		HashMap<String, Rating> myRatings = new HashMap<String, Rating>(); // fill with ratings
		ArrayList<String> items = rater.getItemsRated();

		for (String each : items) {
			Rating rating = new Rating(each, rater.getRating(each));
			myRatings.put(each, rating);
		}
